			...
			<div className='mt-14 px-2'>
				<div className='lg:flex lg:flex-wrap lg:gap-10'>
					...
					<div className='mt-7 lg:w-[45%]'>
						...
						<p className='mt-2 text-paragraph-1 paragraph'>{colorProductDetail.category}</p>
						...
						<div className='mt-2 flex flex-wrap'>
							{colorProductDetail.color.map((data, index) => {
								return (
									<div
										key={index}
										className={`h-8 w-8 lg:h-12 lg:w-12 ${data.hexColor} border-0`}></div>
								);
							})}
						</div>

						<p className='mt-4 text-paragraph-1 paragraph'>
							{isReadMore ? colorProductDetail.desc.slice(0, 150) : colorProductDetail.desc}
							<span onClick={toggleReadMore} className='inline-block text-primary-1 cursor-pointer'>
								{isReadMore ? '... Read More' : ' Show Less'}
							</span>
						</p>

						<h3 className='mt-5 heading text-title-1 lg:text-4xl'>{colorProductDetail.price}</h3>

						<div className='mt-12 grid gap-5 grid-cols-1 grid-rows-1 lg:grid-cols-2 lg:grid-rows-2'>
							<PrimaryButton text='Buy Now' onClick={() => alert('press button')} width='w-full' />
							<SecondaryButton text='Add to Cart' onClick={() => alert('press button')} />
						</div>
					</div>
				</div>
			</div>
