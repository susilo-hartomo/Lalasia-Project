...
function ProductCard({ img, category, name, desc, price, responsiveParent, idx }) {
	...
	return (
		<div
			...
			<div className=''>
				<h5 className={`heading text-paragraph-1 ${responsiveStyle.categoryStyle}`}>{category}</h5>
				<h3 className={`font-bold text-title-1 ${responsiveStyle.nameStyle}`}>{name}</h3>
				<p className={`text-paragraph-1 ${responsiveStyle.descStyle}`}>{desc}</p>
				<h3 className={`heading text-title-1 ${responsiveStyle.priceStyle}`}>{price}</h3>
			</div>
		</div>
	);
}
